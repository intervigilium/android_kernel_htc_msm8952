...
//htc audio ++
static int htc_tomtom_put_micbias_capless(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret;
	...
	if (ucontrol->value.integer.value[0]) {
		ret = snd_soc_update_bits(codec, mc->reg, 0x1E, 0x6);
	} else {
		ret = snd_soc_update_bits(codec, mc->reg, 0x1E, 0x16);
	}

	return 0;
}

static int htc_tomtom_get_micbias_capless(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	u8 capless;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	...
	capless = snd_soc_read(codec, mc->reg);
	capless &= 0x1E;
...
//htc audio ++
	SOC_SINGLE_EXT("MIC_BIAS1_Bypass Cap", TOMTOM_A_MICB_1_CTL, 0, 2, 0, htc_tomtom_get_micbias_capless, htc_tomtom_put_micbias_capless),
	SOC_SINGLE_EXT("MIC_BIAS3_Bypass Cap", TOMTOM_A_MICB_3_CTL, 0, 2, 0, htc_tomtom_get_micbias_capless, htc_tomtom_put_micbias_capless),
//htc audio --
...
static int tomtom_codec_probe(struct snd_soc_codec *codec)
{
	...
	struct snd_soc_dapm_context *dapm = &codec->dapm;
	int ret = 0;
#ifdef USE_CODEC_MBHC //htc audio
	int i, rco_clk_rate;
//htc audio ++
#else //USE_CODEC_MBHC
	int i;
#endif //USE_CODEC_MBHC
//htc audio --
	void *ptr = NULL;
	struct wcd9xxx_core_resource *core_res;
...
//htc audio ++
	...
	snd_soc_update_bits(codec, TOMTOM_A_MBHC_INSERT_DETECT2, 0xC0, 0x00);
	snd_soc_update_bits(codec, TOMTOM_A_MICB_CFILT_2_CTL, 0x01, 0x00);
#endif //USE_CODEC_MBHC
//htc audio --
...
static int tomtom_codec_remove(struct snd_soc_codec *codec)
{
	...
	/* cleanup MBHC */
	//wcd9xxx_mbhc_deinit(&tomtom->mbhc); //HTC_AUD: remove temporarily due to KP
	/* cleanup resmgr */
