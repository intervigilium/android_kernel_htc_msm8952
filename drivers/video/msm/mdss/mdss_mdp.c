...
static int mdss_mdp_parse_dt_pipe(struct platform_device *pdev)
{
    ...
    /*
     * HTC: Avoiding to call asymmertrical API with devm_kzalloc() and kfree().
     * If the memory allocated by calling devm_kzalloc(), it should be freed by call devm_kfree().
     */
parse_fail:
