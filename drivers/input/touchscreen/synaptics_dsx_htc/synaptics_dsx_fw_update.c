		...
		unsigned short block_cnt, unsigned char command)
{
	...
	if (fwu->payload_length > (MAX_TRANSFER_SIZE / fwu->block_size))
		max_transfer = MAX_TRANSFER_SIZE / fwu->block_size;
	else
...
{
	int retval;
	enum flash_area flash_area = UI_NONE;
	unsigned char ii;
	unsigned char config_id_size;
...
static int fwu_get_tw_vendor_v7(void)
{
	...
	retval = fwu_write_f34_partition_id(CMD_READ_SENSOR_ID);
	if (retval < 0)
		return retval;
