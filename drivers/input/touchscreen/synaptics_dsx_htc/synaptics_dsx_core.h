...
#endif
#ifdef MTK_PLATFORM
#define pr_info(fmt, ...) printk(dev_fm(fmt), ##__VA_ARGS__)
#else
...
#endif
#ifdef MTK_PLATFORM
#define pr_notice(fmt, ...) printk(dev_fm(fmt), ##__VA_ARGS__)
#else
...
#endif
#ifdef MTK_PLATFORM
#define pr_err(fmt, ...) printk(dev_err_fm(fmt), ##__VA_ARGS__)
#else
...
#define dev_err(dev, fmt, ...)  do { if (dev) \
			printk(dev_err_fm(fmt), ##__VA_ARGS__); \
			} while (0)
#else
#define dev_err(dev, fmt, ...)  do { if (dev) \
