...
static int mfg_nfc_test(int code)
{
	...
	case 99:
		...
		I("Turn off NFC_PVDD");
#ifdef HIAE_NFC_POWER_CONTROL
        mfc_nfc_cmd_result = regulator_disable(nfc_twl80125_ldo5);
        I("%s : twl80125_ldo5 regulator_disable\n", __func__);
...
static int nfc_irq_pinctrl(struct pn544_dev *pni)
{
	int retval;
	...
	pni->gpio_state_init = pinctrl_lookup_state(pni->pinctrl, "nfc_irq");
		...
		const struct i2c_device_id *id)
{
	...
	I("%s:\n", __func__);
#ifdef HIAE_NFC_POWER_CONTROL
        nfc_twl80125_ldo5 = regulator_get(&client->dev, "twl80125_ldo5");
        I("%s : twl80125_ldo5 regulator_get\n", __func__);
        if (nfc_twl80125_ldo5< 0) {
                E("%s : twl80125_ldo5 regulator_get fail\n", __func__);
                return -ENODEV;
        }
        ret = regulator_set_voltage(nfc_twl80125_ldo5, 1800000, 1800000);
        I("%s : twl80125_ldo5 regulator_set_voltage\n", __func__);
        if (ret < 0) {
                E("%s : twl80125_ldo5 regulator_set_voltage fail\n", __func__);
                return -ENODEV;
        }
        ret = regulator_enable(nfc_twl80125_ldo5);
        I("%s : twl80125_ldo5 regulator_enable\n", __func__);
        I("%s : twl80125_ldo5 regulator_is_enabled = %d\n", __func__, regulator_is_enabled(nfc_twl80125_ldo5));
        if (ret < 0) {
                E("%s : twl80125_ldo5 regulator_enable fail\n", __func__);
                return -ENODEV;
        }
#endif
