...
static u8 virtual_key_led_reset_flag = 0;
...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		led->mpp_cfg->pwm_cfg->lut_params.start_idx = 0;
		led->mpp_cfg->pwm_cfg->lut_params.idx_len = 9;
		led->mpp_cfg->pwm_cfg->lut_params.ramp_step_ms = 66;
...
#ifndef CONFIG_LEDS_LP5562_HTC // conflict with lp5562 driver
void virtual_key_led_reset_blink(int onoff)
{
	...
	if (onoff) {
		...
		if(g_led_virtual->cdev.brightness)
			rc = pwm_config_us(g_led_virtual->mpp_cfg->pwm_cfg->pwm_dev, 640, 640);
		else {
