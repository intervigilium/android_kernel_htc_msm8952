		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 10, (unsigned long *)&set_start_tableIdx);
	if (ret < 0)
		return ret;
...
static void diag_close_logging_process(int pid)
{
	...
	for (i = 0; i < DIAG_NUM_PROC; i++) {
		...
		if (logging_proc->pid != pid) {
			DIAG_DBUG("diag: In %s, logging proc pid %d doesn't match logging proc for %d",
				 __func__, pid, i);
			continue;
		}
		found = 1;
...
static void diag_cmd_invalidate_polling(int change_flag)
{
	...
	if (change_flag == DIAG_CMD_ADD) {
		if (driver->polling_reg_flag) {
			DIAG_DBUG("diag: exiting function %s",__func__);
			return;
		}
	}
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (pkt_type == DCI_PKT_TYPE)
		...
		user_space_data = diagmem_alloc(driver, payload_len,
								POOL_TYPE_USER);
		if (!user_space_data) {
			return -ENOMEM;
		}
		err = copy_from_user(user_space_data, payload_buf,
							 payload_len);
		if (err) {
			pr_err("diag: copy failed for user space data\n");
		...
		diag_process_hdlc_pkt((void *)(user_space_data), payload_len);
