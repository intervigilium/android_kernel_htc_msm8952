					 ...
					 struct usb_configuration *c)
{
	...
	if (!rmnet_initialized) {
		...
		if (dev->pdata->rmnet_transports_interface)
			...
			strcpy(rmnet_transports, "qti,bam2bam_ipa");
			    ...
			    const char *buff, size_t size)
{
	...
	if (usb_disable == 1 && enabled == 1) {
		printk(KERN_WARNING "[USB][%s] Block enable_store(enabled == 1) since usb_disable(%d)\n", __func__, usb_disable);
		mutex_unlock(&dev->mutex);
		return -EPERM;
	}
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	strlcpy(manufacturer_string, "HTC", sizeof(manufacturer_string) - 1);
